...
public class ListenerClass implements Listener {
    ...
    public void onPlayerQuit(PlayerQuitEvent e) {
        ...
        // clear cooldown cache to prevent memory leaks
        BlockHandler.clearCooldown(p.getUniqueId());

        // remove from toggle list to prevent memory buildup over time
    ...
    private void execEvent(String action, CommandSender s, String player, PSRegion region) {
        ...
        if (sp.length < 2)
            return; // need at least action_type and action
